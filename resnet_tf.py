...
slim = tf.contrib.slim # slim是一个使构建，训练，评估神经网络变得简单的库。它可以消除原生tensorflow里面很多重复的模板性的代码，让代码更紧凑，更具备可读性。
...
'''堆叠Blocks的函数，net为输入，bloks为Block类的列表，outputs_collections用来收集各个end_points'''
@slim.add_arg_scope # 用来装饰目标函数,它负责从当前栈中查找用户设置的默认参数, 加上用户传入的调用参数, 调用真正的源函数.
